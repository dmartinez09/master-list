...
export function EstadoBadge({ label }: BadgeProps) {
  const c = ESTADO_COLORS[label] ?? { bg: 'bg-gray-100', text: 'text-gray-600', dot: 'bg-gray-400' };
  return (
...
export function RolBadge({ label }: BadgeProps) {
  const c = ROL_COLORS[label] ?? { bg: 'bg-gray-200', text: 'text-gray-700', border: '' };
  return (
...
export function DimensionBadge({ label }: BadgeProps) {
  const c = (FRAMEWORK_BADGE as Record<string, string>)[label] ?? 'bg-gray-100 text-gray-700 border border-gray-200';
...
export function PrioridadDot({ label }: BadgeProps) {
  ...
  const key = label.startsWith('Alta') ? 'Alta' : label;
  return <span className={`w-2 h-2 rounded-full ${colors[key] ?? 'bg-gray-300'}`} title={`Prioridad: ${label}`} />;
}
...
export function CostoBadge({ label }: BadgeProps) {
  ...
  const cls = map[label.trim()] ?? 'bg-gray-100 text-gray-500';
  return <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${cls}`}>{label.trim() || 'N/D'}</span>;
}
...
export function ImpactoBadge({ label }: BadgeProps) {
  ...
  const key = Object.keys(map).find(k => label.includes(k.substring(0,5))) ?? '';
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${map[key] ?? 'bg-gray-100 text-gray-600'}`}>
      {label.includes('L1') ? 'L1 Estructural' : label.includes('L2') ? 'L2 Habilitador' : 'L3 Mejora'}
    </span>
  );
}
